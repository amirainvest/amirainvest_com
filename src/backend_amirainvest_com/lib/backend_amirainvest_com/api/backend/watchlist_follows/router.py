...
router = APIRouter(prefix="/watchlist_follows", tags=["Watchlist Follows"], dependencies=[Security(auth_dep, scopes=[])])
...
async def update_route(watchlist_follow_data: UpdateModel):
    return (await update_controller(watchlist_follow_data)).dict()
