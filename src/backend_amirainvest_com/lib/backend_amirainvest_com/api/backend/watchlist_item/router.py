...
async def create_route(watchlist_item_data: CreateModel, token=Depends(auth_depends_user_id)):
    ...
    if passable_code == 0:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=Http409Enum.bad_ticker.value.dict())
    elif passable_code == 1:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=Http409Enum.duplicate_ticker.value.dict())
...
@router.post(
    ...
    response_model=GetModel,
    responses={status.HTTP_403_FORBIDDEN: {"model": Http403Model, "description": "Forbidden Request"}},
)
