...
async def work(security: Securities, year: int):
    try:
        ...
        historical_pricing = await get_historical_prices(
            security.ticker_symbol, HistoricalPriceEnum.OneYear
            # f"{year}0101".upper()
        )

        if len(historical_pricing) <= 0:
            failed.append(security.ticker_symbol)
...
async def run(year: int):
    ...
    grouped_securities = group_securities(pruned_securities_list, 10)
...
if __name__ == "__main__":
    run_async_function_synchronously(run, 2022)
    print("FAILURES")
    for f in failed:
        print(f)
